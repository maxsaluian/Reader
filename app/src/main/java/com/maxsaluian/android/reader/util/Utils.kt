...
object Utils {
    ...
    fun copyLinkToClipboard(context: Context, stringUrl: String, view: View? = null) {
        ...
        view?.let {
            Snackbar.make(
                it,
                context.getString(R.string.copied_link),
    ...
    fun setTheme(theme: Int) {
        when (theme) {
            Preferences.THEME_LIGHT -> AppCompatDelegate.MODE_NIGHT_NO
            Preferences.THEME_DARK -> AppCompatDelegate.MODE_NIGHT_YES
    ...
    fun showErrorMessage(view: View, resources: Resources) {
        Snackbar.make(view, resources.getString(R.string.error_message), Snackbar.LENGTH_SHORT)
            .show()
    }
}