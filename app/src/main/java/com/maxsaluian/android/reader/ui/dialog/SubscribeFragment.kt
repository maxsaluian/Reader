...
import com.maxsaluian.android.reader.util.RssUrlTransformer
...
class SubscribeFragment : BottomSheetDialogFragment() {
    ...
    override fun onStart() {
        ...
        val searchResultItem =
            arguments?.getSerializable(ARG_SEARCH_RESULT_ITEM) as SearchResultItem
        val lastUpdated = formatDate(searchResultItem.updated?.toLong())
        Picasso.get().load(searchResultItem.imageUrl).placeholder(R.drawable.feed_icon)
            .into(imageView)
