...
import com.maxsaluian.android.reader.ui.OnHomePressed
import com.maxsaluian.android.reader.ui.OnToolbarInflated
import com.maxsaluian.android.reader.ui.adapter.EntryListAdapter
...
import com.maxsaluian.android.reader.ui.menu.EntryPopupMenu
import com.maxsaluian.android.reader.ui.viewmodel.EntryListViewModel
...
open class EntryListFragment : VisibleFragment(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.setOrder(Preferences.getEntriesOrder(requireContext()))
        viewModel.keepOldUnreadEntries(Preferences.keepOldUnreadEntries(requireContext()))
        autoUpdateOnLaunch = Preferences.getAutoUpdateSetting(requireContext())
    ...
    override fun onResume() {
        super.onResume()
        context?.let { context ->
            viewModel.setOrder(Preferences.getEntriesOrder(context))
            viewModel.keepOldUnreadEntries(Preferences.keepOldUnreadEntries(context))
        }
    }
    ...
    private fun showUpdateNotice() {
        ...
        val itemsAddedString =
            resources.getQuantityString(R.plurals.numberOfNewEntries, count.added, count.added)
        val itemsUpdatedString =
            resources.getQuantityString(R.plurals.numberOfEntries, count.updated, count.updated)
        val message = when {
    ...
    private fun handleShowFeedInfo(feed: Feed?): Boolean {
        return if (feed != null) {
            val mFeed = FeedManageable(
                url = feed.url, title = feed.title, website = feed.website,
    ...
    override fun onRemoveConfirmed() {
        ...
        Snackbar.make(
            recyclerView,
            getString(R.string.unsubscribed_message, title),
            Snackbar.LENGTH_SHORT
        ).show()
    ...
    override fun onEntryClicked(entryId: String, view: View?) {
        if (Preferences.getBrowserSetting(requireContext())) {
    ...
    override fun onStop() {
        super.onStop()
        context?.let { Preferences.saveLastViewedFeedId(it, feedId) }
    }
